...
begin
  // KB4490628 = Servicing Stack Update (required for SHA-2)
...
begin
  ...
  // Win7: check required patches first
  if IsWin7() then
  begin
    MissingPatches := '';
  ...
  // App already installed — offer update vs fresh install
  case MsgBox(
...
begin
  ...
  begin
    ...
    // Refresh icon cache (Win7: -ClearIconCache, Win10: -show)
    if IsWin7() then
